...
class DefaultSessionModule():
    ...
    def updateLayout(self, currentScreen):
        ...
        if channels.channelNumber(True) != -1:
            self.pitchBendRange = channels.getChannelPitch(channels.channelNumber(), 2)
    ...
    def lay0(self):
        ...
        if channels.channelNumber(True) != -1:
            device.midiOutMsg(180, 180, 8, self.knobToCC(channels.getChannelPitch(channels.channelNumber(True))))
